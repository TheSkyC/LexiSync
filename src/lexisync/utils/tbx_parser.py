...
class TBXParser:
    ...
    def parse_tbx(self, filepath: str, analyze_only: bool = False) -> dict:
        ...
        try:
            ...
            logger.info(
                f"Parsed {len(parsed_term_entries)} term entries from {filepath} with languages: {detected_languages}"
            )
            ...
            raise ValueError(f"Invalid XML in TBX file: {e}") from e
        except Exception as e:
            logger.error(f"Unexpected error parsing TBX file {filepath}: {e}", exc_info=True)
            raise
            ...
            if event == "start-ns":
                ...
                if root is None:
                    ...
                    if root_tag not in ("martif", "tbx"):
                        raise ValueError(f"Invalid TBX root element: {root_tag}. Expected 'martif' or 'tbx'.")
    ...
    def _find_term_entries(self, root: ET.Element) -> list[ET.Element]:
        ...
        for name in entry_names:
            ...
            if not found and self.namespace_map:
                for _prefix, uri in self.namespace_map.items():
    ...
    def _extract_lang_map_from_entry(self, entry_node: ET.Element) -> dict[str, list[str]]:
        ...
        for name in lang_section_names:
            lang_sections.extend(entry_node.findall(f"./{name}", self.namespace_map))
        ...
        for sec in lang_sections:
            ...
            if terms:
                ...
                if lang_code in lang_map:
                    lang_map[lang_code].extend(t for t in terms if t not in lang_map[lang_code])
                else:
    ...
    @staticmethod
    def _get_element_text(element: ET.Element) -> str | None:
        """
...
def parse_tbx_file(filepath: str, analyze_only: bool = False) -> dict:
    """
    Parse a TBX file.
