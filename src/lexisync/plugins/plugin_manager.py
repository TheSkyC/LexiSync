...
class PluginManager:
    ...
    def _is_version_compatible(self, app_version, required_prefix):
        ...
        return all(app_parts[i] == req_parts[i] for i in range(len(req_parts)))
    ...
    def load_plugins(self):
        ...
        try:
            ...
            for instance in self.plugins:
                ...
                if failed_deps:
                    self.missing_deps_plugins[instance.plugin_id()] = {"failed_deps": failed_deps}
    ...
    def _sort_and_instantiate_plugins(self, plugin_specs):
            ...
            if missing_deps:
                raise RuntimeError(
                    _("Plugin '{plugin_id}' has missing dependencies: {deps}").format(
                        plugin_id=spec["id"], deps=", ".join(missing_deps)
                    )
                )
        in_degree = dict.fromkeys(all_plugin_ids, 0)
        ...
        if len(sorted_ids) != len(plugin_specs):
            ...
            raise RuntimeError(
                _("Circular dependency detected in plugins: {nodes}. Cannot load.").format(nodes=", ".join(cycle_nodes))
            )
    ...
    def check_dependencies(self, plugin_id_to_check: str) -> bool:
        ...
        if not plugin:
            return False
        enabled_ids = self.main_window.config.get("enabled_plugins", [])
        missing_deps = [dep_id for dep_id in plugin.plugin_dependencies() if dep_id not in enabled_ids]
        if missing_deps:
    ...
    def run_hook(self, hook_name, *args, **kwargs):
        ...
        with plugin_libs_context():
                ...
                for plugin in self.get_enabled_plugins():
                    if hasattr(plugin, hook_name):
                        try:
                            ...
                            logger.error(
                                f"Error in plugin '{plugin.plugin_id()}' processing hook '{hook_name}': {e}",
                                exc_info=True,
                            )
            ...
            if hook_name == "retrieve_context":
                ...
                return flat_list
            if hook_name in [
                "on_file_tree_context_menu",
                "on_table_context_menu",
                "register_resource_viewers",
    ...
    def on_main_app_language_changed(self):
        logger.info("Main app language changed. Reloading all plugins to apply new language...")
        for plugin in self.plugins:
            try:
                plugin.teardown()
            except Exception as e:
    ...
    def is_dependency_for_others(self, plugin_id_to_check: str) -> bool:
        ...
        if dependent_plugins:
            ...
            msg = _("Cannot disable '{plugin_name}'.\nThe following enabled plugins depend on it:\n\n- {deps}").format(
                plugin_name=plugin_to_disable.name(), deps="\n- ".join(dependent_plugins)
            )
            QMessageBox.warning(self.main_window, _("Dependency Error"), msg)
            return True
        return False
