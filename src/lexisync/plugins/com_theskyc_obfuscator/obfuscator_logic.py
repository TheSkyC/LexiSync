...
class ObfuscatorLogic:
    ...
    def _parse_and_separate_blocks(self, content):
        ...
        self.rules_block = ""
        rule_start_match = regex.search(
            r"^\s*(?:禁用\s*|disabled\s*)?规则|rule", content, regex.MULTILINE | regex.IGNORECASE
        )
        if rule_start_match:
        ...
        if global_vars_match:
            for line in global_vars_match.group(1).splitlines():
                if ":" in line:
                    ...
                    name = parts[1].strip()
                    if name:
                        self.dynamic_list["global_vars"].append(name)
        ...
        if player_vars_match:
            for line in player_vars_match.group(1).splitlines():
                if ":" in line:
                    ...
                    name = parts[1].strip()
                    if name:
                        self.dynamic_list["player_vars"].append(name)
        ...
        if subroutines_match:
            for line in subroutines_match.group(2).splitlines():
                if ":" in line:
                    ...
                    name = parts[1].strip()
                    if name:
                        self.dynamic_list["subroutines"].append(name)
    ...
    def _string_replacer(self, match):
        ...
        for i, part in enumerate(parts):
            if i % 2 == 1:
                ...
                obfuscated_part = "".join(chr(ord(c) + 0xE0000) if " " <= c <= "~" and c != "\\" else c for c in part)
    ...
    def _obfuscate_names(self, rules_text):
        ...
        if all_var_names:
            prefix_pattern = regex.compile(
                r"((?:[\w\u4e00-\u9fa5]+|\))\.)(" + "|".join(regex.escape(n) for n in all_var_names) + r")\b",
                regex.IGNORECASE,
            )
        ...
        if global_map:
            ...
            global_func_pattern = regex.compile(
                r"((?:修改全局变量|设置全局变量|在索引处设置全局变量|在索引处修改全局变量|持续追踪全局变量|追踪全局变量频率|停止追踪全局变量|For\s+全局变量)\s*\()("
                + "|".join(regex.escape(n) for n in global_names)
                + r")([,\)])",
                regex.IGNORECASE,
            )
        ...
        if player_map:
            ...
            player_func_pattern = regex.compile(
                r"((?:修改玩家变量|设置玩家变量|在索引处设置玩家变量|在索引处修改玩家变量|持续追踪玩家变量|追踪玩家变量频率|停止追踪玩家变量|For\s+玩家变量)\s*\((?:[^,)]+,)*\s*)("
                + "|".join(regex.escape(n) for n in player_names)
                + r")([,\)])",
                regex.IGNORECASE,
            )
        ...
        if sub_map:
            ...
            rules_text = event_pattern.sub(
                lambda m: m.group(1) + sub_map.get(m.group(2), m.group(2)) + m.group(3), rules_text
            )
            # C-2: `调用子程序(SubName)`
            call_pattern = regex.compile(
                r"(调用子程序\s*\()(" + "|".join(regex.escape(n) for n in sub_names) + r")(\s*\))", regex.IGNORECASE
            )
            rules_text = call_pattern.sub(
                lambda m: m.group(1) + sub_map.get(m.group(2), m.group(2)) + m.group(3), rules_text
            )
            # C-3: `开始规则(SubName, ...)`
            ...
            rules_text = start_rule_pattern.sub(
                lambda m: m.group(1) + sub_map.get(m.group(2), m.group(2)) + m.group(3), rules_text
            )
