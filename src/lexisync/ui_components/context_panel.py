...
from PySide6.QtGui import QColor, QFont, QFontMetricsF, QTextCharFormat, QTextCursor
...
class ContextPanel(QWidget):
    ...
    def set_context(self, ts_obj):
        ...
        if 0 <= current_line_in_context_idx < len(context_lines):
            ...
            if not keyword_cursor:
                ...
                try:
                    ...
                    keyword_cursor.setPosition(
                        current_block.position() + start_index + len(keyword_to_highlight), QTextCursor.KeepAnchor
                    )
                except ValueError:
                    pass
            ...
            target_y = (
                self.context_text_display.verticalScrollBar().value()
                + cursor_rect.top()
                - (viewport_height / 2)
            ...
            target_x = (
                self.context_text_display.horizontalScrollBar().value()
                + cursor_rect.left()
                - (viewport_width / 2)
                + (cursor_rect.width() / 2)
            )
    ...
    def update_ui_texts(self):
        self.findChild(QLabel, "context_label")
