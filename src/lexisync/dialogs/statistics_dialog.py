...
from PySide6.QtCharts import QBarCategoryAxis, QBarSet, QChart, QChartView, QHorizontalStackedBarSeries, QValueAxis
from PySide6.QtCore import QEasingCurve, QMargins, QModelIndex, QPropertyAnimation, Qt, QThread, QTimer, Signal
...
class StatisticsDialog(QDialog):
    ...
    def _display_metrics(self, stats_data):
        ...
        self.add_metric_with_progress(
            _("Untranslated"), f"{untranslated} ({untranslated_p:.1%})", untranslated_p, "#F44336"
        )
        ...
        self.add_metric_with_progress(
            _("Reviewed (of total)"), f"{reviewed} ({reviewed_p_total:.1%})", reviewed_p_total, "#4CAF50"
        )

        reviewed_p_translated = reviewed / translated if translated else 0
        self.add_metric_with_progress(
            _("Reviewed (of translated)"), f"{reviewed} ({reviewed_p_translated:.1%})", reviewed_p_translated, "#2196F3"
        )
        ...
        self.add_metric_with_progress(
            _("Items with Warnings"), f"{warning_count} ({warning_p:.1%})", warning_p, "#FF9800"
        )
    ...
    def _add_warning_category(self, warning_type, ts_obj_list):
        ...
        for ts_obj in ts_obj_list:
            ...
            issue_item_line1.setToolTip(f"Original: {ts_obj.original_semantic}\n\n{_('Double-click to locate.')}")
